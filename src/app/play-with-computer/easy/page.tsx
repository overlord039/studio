...
type EasyModeFormValues = z.infer<typeof easyModeFormSchema>;
...
export default function EasyModePage() {
  ...
  const form = useForm<EasyModeFormValues>({
    resolver: zodResolver(easyModeFormSchema),
    defaultValues: {
      numberOfTickets: 1,
    },
  });

  async function onSubmit(values: EasyModeFormValues) {
    if (!currentUser) {
    ...
    try {
      const response = await fetch('/api/bot-game/create', {
        ...
        body: JSON.stringify({ host: hostPlayer, mode: 'easy', tickets: values.numberOfTickets }),
      });
