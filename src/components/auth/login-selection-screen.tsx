...
export default function LoginSelectionScreen() {
  const { loginWithGoogle, loginAsGuest, loginWithFacebook } = useAuth();
  const [isSigningIn, setIsSigningIn] = useState<null | 'google' | 'guest' | 'facebook'>(null);
      ...
      <Card className="w-full max-w-sm text-center animate-fade-in-up bg-black/50 backdrop-blur-md border border-white/20 relative">
        <CardContent className="p-8 space-y-4">
          ...
          <div className="space-y-3 pt-4">
            ...
            <Button className="w-full bg-[#1877F2] text-white hover:bg-[#166fe5] shadow-sm" size="lg" onClick={handleFacebookSignIn} disabled={!!isSigningIn}>
              {isSigningIn === 'facebook' ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : <FacebookIcon className="mr-2 h-5 w-5" />}
              {isSigningIn === 'facebook' ? "Redirecting..." : "Sign in with Facebook"}
            </Button>
          </div>
